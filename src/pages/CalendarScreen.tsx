...
import type { WorkoutData, WorkoutSession, ExerciseEntry } from '../types'
...
// ── Sub-component: one exercise block in the detail card ─────────────────────
function ExerciseBlock({ entry }: { entry: ExerciseEntry }) {
  const isCardio = entry.category === '有酸素'
  return (
    <div className="py-2.5 px-4">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-sm font-semibold text-white">{entry.name}</span>
        <span className="text-[10px] text-muted bg-surface px-1.5 py-0.5 rounded-md">{entry.category}</span>
      </div>
      <div className="space-y-1">
        {entry.sets.map((set, j) => (
          <div key={set.id} className="flex items-center gap-2 text-xs">
            ...
            {set.memo && (
              <span className="text-muted truncate">— {set.memo}</span>
            )}
          </div>
        ))}
      </div>
...
// ── Sub-component: one session card ──────────────────────────────────────────
      ...
      <div className="px-4 py-3 bg-surface/60 flex items-center justify-between">
        <div>
          <div className="text-sm font-bold text-white">
            ...
            {duration && (
              <span className="text-xs text-muted font-normal ml-2">({duration})</span>
            )}
          </div>
...
// ── Main component ────────────────────────────────────────────────────────────
export default function CalendarScreen({ data }: Props) {
              ...
              className={`
                ...
                ${isSelected
                  ...
                  : isToday
                    ...
                    : hasWorkout
                      ? 'bg-card border border-border active:bg-surface'
        ...
        <div className="px-4 mt-4 slide-in">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-bold text-white">
              {selectedDate.replace(/-(\d+)-(\d+)$/, (_, m, d) => `${Number(m)}月${Number(d)}日`)}のワークアウト
            </span>
